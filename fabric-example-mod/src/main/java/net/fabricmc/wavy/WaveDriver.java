...
public class WaveDriver {
    ...
    //Lists all the blocks that we have seen, and so should be in the list 
    Vector<Block> listOfSeenBlocks = new Vector<Block>();
    int currentIndex = 0;
    ...
    */
    ...
    private void addAdjacencies(Block b, BlockPos p){
        ...
        testSymbolSeenBefore(newPos, newBlock);
    ...
    private void testSymbolSeenBefore(BlockPos pos, Block b){
        if(!listOfSeenBlocks.contains(b)){ //Test if the block has been seen before, if it hasn't we need to add it
            integerToBlockMap.put(++currentIndex, b);
            blockToIntegerMap.put(b, currentIndex);
            listOfSeenBlocks.add(b); 
